...
let isFilteringCrossposts = true;
...
async function removeOldEntries() {
    ...
    const { seenPostsSubreddit = {}, seenPostsID = {} } = await new Promise((resolve) => chrome.storage.local.get(["seenPostsSubreddit", "seenPostsID"], resolve));
    ...
    if (changesToSubreddit) {
        await new Promise((resolve) => chrome.storage.local.set({ seenPostsSubreddit: newSeenPostsSubreddit }, resolve));
    }
    if (changesToID) {
        await new Promise((resolve) => chrome.storage.local.set({ seenPostsID: newSeenPostsID }, resolve));
    }
}
